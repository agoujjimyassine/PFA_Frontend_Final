...
@Component({
  selector: 'app-pieces-detaches-commande',
  templateUrl: './pieces-detaches-commande.component.html',
  styleUrls: ['./pieces-detaches-commande.component.css']
})
export class PiecesDetachesCommandeComponent implements OnInit {
  ...
  constructor(private pieceDetacheService: ServicePieceDetache, private router: Router,
  ...
  ngOnInit() {
    ...
    this.pieceDetacheService.getPieceDetachesByIdCommande(this.id)
      .subscribe(data => {
  ...
  onEditPieceDetache(id: number) {
    this.router.navigate(['editer-piece-detache-commande', id]);
  }
  ...
  addPieceDetache(id: number) {
    this.router.navigate(['ajouter-piece-detache-commande', id]);
  }
}
